...
final readonly & models:SmartConfiguration smartConfiguration = check generateSmartConfiguration().cloneReadOnly();
...
service / on new http:Listener(9091) {
    ...
    resource isolated function get fhir/r4/\.well\-known/smart\-configuration() returns json|r4:FHIRError {
        ...
        if response is json {
            utils:LogDebug("Smart configuration served at " + time:utcNow()[0].toString());
            return response;
        } else {
