...
function testService() returns error? {
    ...
    json|error response = testClient->get("/fhir/r4/.well-known/smart-configuration");
    if response is json {
...
function testServiceWithSampleError() returns error? {
    http:Client testClient = check new ("http://localhost:9090");
    http:Response response = check testClient->post("/fhir/r4/.well-known/smart-configuration", message = "");
